...
function getStaticErrors(declarations) {
  ...
  declarations.forEach(decl => {
    ...
    if (decl.funcao) {
      functions.push(decl);
    } else if (decl.tipoOriginal !== undefined || decl.constructor?.name === 'Var') {
  ...
  functions.forEach(funcDecl => {
    ...
    function collectLocalDeclarations(stmt) {
      ...
      const isVar = name === 'Var' || stmt.tipoOriginal !== undefined || (stmt.inicializador !== undefined && stmt.tipoOriginal !== undefined);
